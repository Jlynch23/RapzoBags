        ...
        local function showTooltip(button)
            if MerchantFrame.selectedTab == 1 then
                ...
                if itemID then
                    ...
                    if collected then
                        GameTooltip:AddLine(string.format("Rapzo Bags: OBTENIDO%s", kind and (" - " .. kind) or ""), 0.22, 0.95, 0.40)
                    end
                    if owned > 0 then
                        GameTooltip:AddLine(string.format("Rapzo Bags: tienes %d", owned), 0.22, 0.75, 1.00)
                    end
                end
                GameTooltip:Show()
            else
...
function Vendor:SetupMerchantFrame()
    ...
    if isKrowiLoaded() then
        if not self.conflictWarned then
            ...
            RB:Print("Krowi Extended Vendor UI esta activo. Desactivo el vendedor extendido de Rapzo Bags para evitar conflictos.")
        end
        return false
    end
...
function Vendor:HandleSlash(rest)
    ...
    elseif first == "collected" or first == "obtenidos" then
        ...
        elseif value == "off" then
            ...
            RB:Print("Uso: /rbags vendor obtenidos on|off")
        end
        return true
    ...
    if first == "grid" then
        ...
        if columns and rows then
            ...
            RB:Print("Uso: /rbags vendor 4x5")
        end
        return true
    end

    RB:Print("Uso: /rbags vendor [status|on|off|4x5|filter all|uncollected|mounts|transmog|recipes|obtenidos on|off|reset]")
    return true
end
...
RB:RegisterCommand("vendor", function(rest) Vendor:HandleSlash(rest) end, "/rbags vendor [status|on|off|4x5|filter ...] - configura el vendedor")
Vendor:Initialize()
