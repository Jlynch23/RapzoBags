...
function Search:CreateFrame()
    ...
    frame.title:SetText("Rapzo Bags - Buscador global")
...
function Search:Show(query)
    if not RB:IsFeatureEnabled("search") then
        RB:Print("El modulo Search esta desactivado en Rapzo Bags Config.")
        return
    end
    local frame = self:CreateFrame()
    frame:Show()
...
function Search:Initialize()
    ...
    RB:RegisterCommand("search", function(rest) Search:Show(rest) end, "/rbags search <texto> - busca en todos tus personajes y bancos")
    RB:RegisterCommand("find", function(rest) Search:Show(rest) end)
    RB:RegisterCommand("gold", function() Search:ShowGold() end, "/rbags gold - muestra el oro guardado por personaje")
